...
def newsletter_data(doc):
  ...
  return  {"articles": PAGES, "date": DATE, "title": TITLE, "heading": HEADING, "markdown": MARKDOWN, "HTML": HTML, "description": doc["description"],  "links": []}




mc = MailChimp(
    mc_api = os.environ["MAILCHIMP_API"], 
    mc_user = os.environ["MAILCHIMP_USER"])
...
def create_newsletter(DATA, HTML):
  ...
  mc.campaigns.actions.test(C["id"], {
    "send_type": "html",
    "test_emails":["swamidass@gmail.com"]
  })
